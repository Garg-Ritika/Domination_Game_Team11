...
public class HumanPlayer implements OrderInterface {
    ...
    public HumanPlayer(Player p_Initiator, Territory p_Source, Territory p_Target, int p_Num) {
    ...
    public HumanPlayer(Player p_Initiator, Territory p_TargetTerritory) {
        this.d_Target = p_TargetTerritory;
        this.d_Initiator = p_Initiator;
    }

    public HumanPlayer(Player p_Initiator, Territory p_Target_territory, int p_To_deploy) {
    ...
    public void airlift() {
        ...
        if ((d_Source.getArmyCount() >= d_Num)
        ...
        if (L_Valid_condition) {
            // if the source and the target belong to the same player
            // then just move the armies to the target Territory
            d_Target.setArmyCount(d_Target.getArmyCount() + d_Num);
            d_Source.setArmyCount(d_Source.getArmyCount() - d_Num);
            LogUtil.log(d_Num + " has been moved");
        } else {
            LogUtil.log("invalid order");
        }
    ...
    public void deploy() {
        ...
        if (L_Valid_condition) {
            // behavior of the concrete command
            int l_ExistingArmy = this.d_Target.getArmyCount();
            this.d_Initiator.setNoOfArmies(this.d_Initiator.getNoOfArmies() - d_Num);
            this.d_Target.setArmyCount(l_ExistingArmy + d_Num);
            LogUtil.log("deploying " + d_Num + " to " + d_Target.getName());
        }
